    ...
    schema.pre('save', function (next) {
      ...
      Object.keys(options.fields).forEach((field) => {
        ...
        if (options.fields[field].array) {
          ...
          // HACK: if field is 'resume', don't remove old resumes
          // (because there's an ongoing bug that resume gets deleted even if it's not updated)
        } else if (model.isModified(field) && field !== 'resume') {
